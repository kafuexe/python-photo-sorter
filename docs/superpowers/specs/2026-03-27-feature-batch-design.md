...
**Date:** 2026-03-27
**Scope:** 4 new features for Photo Sorter
**Status:** Draft
...
Four features that extend the photo sorter's extraction coverage, add a non-destructive preview mode, and improve the processing UI. Each feature is independent and can be implemented/shipped separately, except Feature 4 which depends on Feature 3's status counters.
...
- `SamsungHandler` uses a strict end-of-string anchor (`$`) to avoid false positives on arbitrary numeric filenames. Note: the `YYYYMMDD_HHMMSS` pattern is used by many cameras and devices beyond Samsung — the handler name reflects the most common source, but it intentionally acts as a catch-all for this pattern
- `DatestampHandler` at priority 15 runs after device-specific handlers — it's a generic "looks like a date" fallback. Its regex intentionally has no end-of-string anchor, allowing matches like `2024-06-15 14.30.00 extra text.jpg` for maximum coverage
...
6. Wrap all of the above in try/except — `MediaInfo.parse()` can raise native-library errors. On any exception, log at debug level and return `{}`

**Note on `File_Modified_Date`:** This is a deliberate last-resort fallback. Filesystem modification dates change when files are copied or downloaded, so they are unreliable — but for files with no `Tagged_Date` or `Encoded_Date`, an approximate date is better than routing to `.unknown`.
...
Changes span `app_window.py` (widget creation) and `ProcessingController` (logic):

**`app_window.py`** — create 3 new `ctk.CTkLabel` widgets in the progress frame (timing, breakdown, current file) following the existing pattern where `app_window` creates widgets and passes them to the controller.
...
- `src/ui/app_window.py` (modified — add timing, breakdown, current file labels to progress frame)
- `src/ui/processing_controller.py` (modified — stat tracking and label updates)

---

## Feature 4: Dry Run with Tree Preview

**Depends on:** Feature 3 (reuses status breakdown counters for the stats summary)
...
- When `config.dry_run` is True, the executor call is skipped entirely. The modified control flow:

```python
if dest is None:
    result.status = "skipped"
elif config.dry_run:
    # Assume executor would succeed — assign status based on metadata
...
The UI context (dry run) communicates that nothing was actually moved/copied.
...
- In the processing loop, check `cancel_event.is_set()` after each file — if set, break out of the loop and return results processed so far (partial results list)
- Reset the event at the start of each run
- The stop button is **only available during dry runs** — regular copy/move operations cannot be cancelled (cancelling mid-move could leave files in an inconsistent state)
- `ProcessingService.process()` returns a `(results, stats)` tuple as normal. The caller determines whether the run was cancelled by checking `cancel_event.is_set()` after process returns
- The results list is safe to return partially because each result is appended atomically (single thread in the processing loop)
- When cancelled, `ProcessingController` shows the summary as "Dry run stopped — would sort N files, ..." instead of "Would sort N files, ..."

**Button mutual exclusion:**
- During any active operation (dry run, copy, or move), all three action buttons are disabled
- The dry run button's text changes to "Stop" only during a dry run, and reverts when complete or stopped
...
Note: `FileStatHandler` (priority 100, wildcard `*` extension) exists in the codebase but is intentionally **not registered** in `__main__.py`. It uses filesystem timestamps as a last resort, which are unreliable. If it is ever registered, it should go after all handlers listed above.
